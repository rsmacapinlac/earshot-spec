...
logged to the systemd journal ([ADR-0004](0004-systemd-for-service-management.md)).

See [specs/storage.md](../specs/storage.md#filesystem-as-state) for the
authoritative as-built state table.
...
> **As-built:** a small `status.json` per session (written for the companion
> `earshot-tui`) mirrors the derived state (`encoded` → `transcribed`) but is
> **not** the source of truth — file presence is. See
