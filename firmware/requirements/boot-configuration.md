...
earshot reads a small user-editable configuration file from the SD card at boot.
...
user-facing mechanism remains `/earshot.cfg`; the eventual hands-off source is the
...
The future BLE companion app should treat `/earshot.cfg` as the device's stable
...
The app may update `/earshot.cfg` directly through a future config/transfer seam,
or use an equivalent device-side API that persists changes back to this file.
Either way, `/earshot.cfg` remains the on-device source of truth for these v1-era
