...
This document defines the initial implementation acceptance criteria for earshot
...
| ID | Status | Criterion | Source | Verification |
| -- | ------ | --------- | ------ | ------------ |
| AC-TIME-01 | Final | Firmware reads `/earshot.cfg` from SD root at boot if available. | `boot-configuration.md` | Boot with config; verify parsed values. |
| AC-TIME-02 | Final | Missing/unreadable `/earshot.cfg` is not fatal; safe defaults are used. | `boot-configuration.md` | Boot without file. |
...
| AC-TIME-09 | Final | Future app path can read/update `/earshot.cfg` or call an API that persists changes back to that file. | `boot-configuration.md`, ADR 0007 | Interface/API review; v1 may stub. |
...
| AC-SYNC-03 | Final | The seam/interface leaves room for future `/earshot.cfg` read/update or equivalent config API persisted to that file. | ADR 0007, `boot-configuration.md` | Interface review. |
...
| ID | Status | Criterion | Source | Verification |
| -- | ------ | --------- | ------ | ------------ |
| AC-INT-01 | Provisional | Full sketch compiles for `esp32s3` profile with pinned dependencies. | `sketch.yaml` | `arduino-cli compile --profile esp32s3 earshot`. |
...
3. Exact `/earshot.cfg` mark-applied strategy for `rtc_set_utc`.
