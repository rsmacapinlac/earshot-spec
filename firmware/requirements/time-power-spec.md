...
Timekeeping, battery, and power/sleep requirements for earshot, extracted from
the reference firmware, which runs on the same **Waveshare ESP32-S3 1.54" e-Paper**
board earshot targets. These are the specs a compatible implementation must honor
when these subsystems are wired into earshot.
...
- **Local time for display only:** applied when rendering labels; **storage stays
  UTC**. earshot makes this offset configurable via the SD-card boot config
  (`/earshot.cfg`, key `local_offset_min`) — see `boot-configuration.md` and ADR
  `../adr/0005-time-model.md`.
- **Manual RTC bootstrap:** v1 has no on-device settings screen. A valid optional
  `rtc_set_utc=` entry in `/earshot.cfg` may be used at boot to set the system
...
> The above describes the reference firmware's **deep** sleep. earshot v1 uses
